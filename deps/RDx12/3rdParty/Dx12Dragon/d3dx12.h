...
{
    UINT64 RequiredSize = 0;
    //这里为什么要sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT) + sizeof(UINT) + sizeof(UINT64)？因为每个subres需要三个描述符内存：一个D3D12_PLACED_SUBRESOURCE_FOOTPRINT，一个pNumRows(UINT)。一个pRowSizesInBytes（UINT64）
    UINT64 MemToAlloc = static_cast<UINT64>(sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT) + sizeof(UINT) + sizeof(UINT64)) * NumSubresources;//*NumSubresources表示所有子资源描述符的总内存
    if (MemToAlloc > SIZE_MAX)
    {
       return 0;
    }
    void* pMem = HeapAlloc(GetProcessHeap(), 0, static_cast<SIZE_T>(MemToAlloc));//申请总内存
    if (pMem == nullptr)
    {
       return 0;
    }

    auto pLayouts = reinterpret_cast<D3D12_PLACED_SUBRESOURCE_FOOTPRINT*>(pMem);//将总内存解释为第一个描述符数组(数组长度>NumSubresources，因为还包括了二三描述符的内存)
    UINT64* pRowSizesInBytes = reinterpret_cast<UINT64*>(pLayouts + NumSubresources);//取第一个描述符数组偏移NumSubresources位,并将其转化位UINT64指针，意思是将除了sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT)*NumSubresources后面的内存看做第二个描述符数组，即UINT64数组
    UINT* pNumRows = reinterpret_cast<UINT*>(pRowSizesInBytes + NumSubresources);//取第二个描述符数组偏移NumSubresources位,并将其转化位UINT指针，意思是将除了sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT)*NumSubresources，UINT64*NumSubresources后面的内存看做第三个描述符，即UINT数组
    
                                                                                 
    //最后的内存分布情况
    //总内存长度：（sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT) + sizeof(UINT) + sizeof(UINT64)) * NumSubresources;
    //|------------------------------|-------------------|------------------|
    //playouts指向的内存长度：sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT)* NumSubresources
    //playouts代表的意义D3D12_PLACED_SUBRESOURCE_FOOTPRINT[NumSubresources]中下标为1的数据
    //pRowSizesInBytes指向的内存长度UINT64* NumSubresources;代表UINT64[NumSubresources]
    //pNumRows指向的内存长度UINT* NumSubresources;代表UINT[NumSubresources]
    //其实上面写了一大堆就是动态声明了三个数组
    //pLayouts=new D3D12_PLACED_SUBRESOURCE_FOOTPRINT[NumSubresources]; //用于装子资源的footsprint描述符，包括子资源的width,height,depth,height,rowpitch等，rowpitch是指一行数据的物理尺寸rowpitch=width*bytesperpix,但注意这里的rowpitch有对齐要求， This must be a multiple of D3D12_TEXTURE_DATA_PITCH_ALIGNMENT (256),
    //pRowSizesInBytes=new UINT64[NumSubresources];//每个元素表示对应子资源的行物理大小，即pRowSizeInBytes=width*bytesperpix，与rowpitch的区别是没有内存字节对齐要求
    //pNumRows=new UINT[NumSubresources];//每个元素表示对应的子资源的行数
    //本函数是通过动态申请内存来生成动态大小的数组，对应下面模板类函数通过模板来动态声明数组
...
{
    UINT64 RequiredSize = 0;
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT Layouts[MaxSubresources];//通过模板来动态声明数组
