...
struct IMGUI_API ImPool
{
    ...
    ImPoolIdx       FreeIdx;    // Next free mMeshIdx to use
...
struct ImGuiContext
{
    ...
    float                   FontBaseSize;                       // (Shortcut) == IO.FontGlobalScale * Font->mScale * Font->FontSize. Base text height.
...
struct IMGUI_API ImGuiWindow
{
    ...
    // The best way to understand what those rectangles are is to use the 'Metrics -> Tools -> ShowWin windows rectangles' viewer.
    ...
    int                     MemoryDrawListIdxCapacity;          // Backup of last mMeshIdx/vtx count, so when waking up the window we can preallocate and avoid iterative alloc/copy
...
enum ImGuiTabBarFlagsPrivate_
{
    ...
    ImGuiTabBarFlags_SaveSettings               = 1 << 22   // FIXME: Settings are handled by the docking system, this only request the tab bar to mark settings mDirty when reordering tabs
};
