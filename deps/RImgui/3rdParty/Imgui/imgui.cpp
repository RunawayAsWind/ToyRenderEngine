...
/*
 ...
 - 2017/08/20 (1.51) - added PushStyleColor(ImGuiCol mMeshIdx, ImU32 col) overload, which _might_ cause an "ambiguous call" compilation error if you are using ImColor() with implicit cast. Cast to ImU32 or ImVec4 explicily to fix.
...
ImGuiStyle::ImGuiStyle()
{
    ...
    MouseCursorScale        = 1.0f;             // mScale software rendered mouse cursor (when io.MouseDrawCursor is enabled). May be removed later.
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    float dby = NavScoreItemDistInterval(ImLerp(cand.Min.y, cand.Max.y, 0.2f), ImLerp(cand.Min.y, cand.Max.y, 0.8f), ImLerp(curr.Min.y, curr.Max.y, 0.2f), ImLerp(curr.Min.y, curr.Max.y, 0.8f)); // mScale down on Y to keep using box-distance for vertically touching items
                ...
                while (clipper.Step())
                    for (int prim = clipper.DisplayStart, idx_i = elem_offset + clipper.DisplayStart * 3; prim < clipper.DisplayEnd; prim++)
                    {
                        ...
                        for (int n = 0; n < 3; n++, idx_i++)
                        {
                            ...
                            triangle[n] = v.pos;
                            buf_p += ImFormatString(buf_p, buf_end - buf_p, "%s %04d: mPos (%8.2f,%8.2f), uv (%.6f,%.6f), col %08X\n",
