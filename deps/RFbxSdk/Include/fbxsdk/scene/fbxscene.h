...
public:
	...
	* \param pPoseArray An array of pose pointers. */
	...
	/** \name Utilities */
	//@{
		/** Synchronize all the ShowWin properties of node instances.
		* Walks all the node attributes defined in the scene and synchronize the ShowWin property of all the nodes that reference the node attribute so that they all contain the same value.
		...
		* \remarks Applications only need to call this method if their interpretation of the ShowWin property implies that setting the ShowWin state on one instance affect all of them.
		* \see FbxNode::Visibility property, FbxNode::ShowWin property */
