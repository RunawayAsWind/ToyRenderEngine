...
/** Represents an element in the scene graph. A scene graph is a tree of FbxNode 
  ...
  * the position of the object in the scene. This information include the basic mTrans,
...
public:
	...
	//@{
		...
		/** Reset all the pivot sets to the default pivot context and convert the animation.
		  ...
		  * \param pForceResetLimits  If \c true, this flag will reset all the mTrans, Rotation and Scaling 
	...
	//@{
		...
		* node only.
		*
		* To access this property do: ShowWin.Get().
		* To set this property do: ShowWin.Set(FbxBool).
		* 
		* Default value is true.
		*
		* \remarks  \li Because node attributes can be shared by multiple nodes (instances), the FBX SDK provides an utility
		*           function FbxScene::SyncShowPropertyForInstance() to propagates the same ShowWin value across all the nodes
		*           referencing the node attribute. The applied logic is that as soon as one of these nodes has the ShowWin 
		*           property set to \c false, all will be set to \c false (basically it is an AND operation on all the
		*           ShowWin flags).
		*
		*           \li Depending on the support of the ShowWin and Visibility properties that applications will implement, there 
		*           may be conflicts with these two states. In this case, it is suggested that the Visibility property
		*           always overrides the ShowWin.
		*
		* \see Visibility property.
		*/
